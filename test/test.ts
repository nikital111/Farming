...
describe("Farming", function () {
  ...
  it("claims", async function () {
    ...
    const time1 = (await ethers.provider.getBlock((await claim1Tx).blockNumber))
      .timestamp;
    ...
    const time2 = (await ethers.provider.getBlock((await claim2Tx).blockNumber))
      .timestamp;

    // find and verify event
    const rc2 = await claim2Tx.wait(); // 0ms, as tx is already confirmed
